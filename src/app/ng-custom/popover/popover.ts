...
import { NgcPopoverConfig } from './popover-config';

let nextId = 0;

@Component( {
    selector: 'ngc-popover-window',
 ...
 */
@Directive( { selector: '[ngcPopover]', exportAs: 'ngcPopover' } )
export class NgcPopover implements OnInit, OnDestroy, OnChanges {
    /**
     ...
     */
    @Input() ngcPopover: string | TemplateRef<any>;
    /**
    ...
    /**
     * An optional class applied to ngc-popover-window
     *
     * @since 2.2.0
     */
    @Input() popoverClass: string;
    ...
    private _ngcPopoverWindowId = `ngc-popover-${nextId++}`;
    private _popupService: PopupService<NgcPopoverWindow>;
    private _windowRef: ComponentRef<NgcPopoverWindow>;
    ...
    private _isDisabled(): boolean {
        ...
        if ( !this.ngcPopover && !this.popoverTitle ) {
            return true;
        }
        return false;
    }
    ...
    constructor(
        ...
        componentFactoryResolver: ComponentFactoryResolver, viewContainerRef: ViewContainerRef, config: NgcPopoverConfig,
        ...
        this._popupService = new PopupService<NgcPopoverWindow>(
            NgcPopoverWindow, injector, viewContainerRef, _renderer, componentFactoryResolver );
    ...
    open( context?: any ) {
        if ( !this._windowRef && !this._isDisabled() ) {
            this._windowRef = this._popupService.open( this.ngcPopover, context );
            this._windowRef.instance.title = this.popoverTitle;
            this._windowRef.instance.context = context;
            this._windowRef.instance.popoverClass = this.popoverClass;
            this._windowRef.instance.id = this._ngcPopoverWindowId;

            this._renderer.setAttribute( this._elementRef.nativeElement, 'aria-describedby', this._ngcPopoverWindowId );
    ...
    ngOnChanges( changes: SimpleChanges ) {
        ...
        if ( (changes['ngcPopover'] || changes['popoverTitle'] || changes['disablePopover']) && this._isDisabled() ) {
            this.close();
        }
    }
